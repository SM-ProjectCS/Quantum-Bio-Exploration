...
Le fichier `quantum_search.py` est une simulation. Voici comment cela fonctionne simplement :
...
* **L'algorithme de Grover** trouve la carte en **une seule opération**.

### Comment ça marche ? (La notion d'Amplitude)
En quantique, on ne parle pas juste de 0 ou 1, mais "d'amplitude de probabilité". Imaginez cela comme le volume sonore d'une piste audio.
1.  **Superposition :** Au début, toutes les cartes ont le même "volume". L'ordinateur les considère toutes égales.
2.  **L'Oracle (Le Marquage) :** L'ordinateur repère l'As de Cœur mais ne nous le montre pas. À la place, il inverse sa phase (il met son volume en "négatif").
3.  **L'Amplification (Inversion autour de la moyenne) :** C'est une opération mathématique complexe. L'ordinateur utilise cette valeur "négative" pour transférer tout le "volume" des mauvaises cartes vers l'As de Cœur.
...
J'utilise **Qiskit** (IBM) avec un simulateur (`Aer`). C'est un programme classique qui imite un ordinateur quantique.
...
* **Mais il est limité :** Pour simuler un système quantique, il faut stocker des nombres complexes pour chaque état possible.
    ...
    * Simuler 50 qubits = Impossible sur un ordinateur classique. Cela demanderait des **Pétaoctets de RAM** (plus que la mémoire de tous les supercalculateurs du monde réunis).

C'est la raison pour laquelle la construction de vrais ordinateurs quantiques est nécessaire : pour dépasser cette limite physique de la mémoire classique.
